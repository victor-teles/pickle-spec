...
/** Matches Stagehand v4 onLog payload (not currently re-exported by the SDK). */
export type StagehandLogLine = {
...
export function reportSummary(result: RunResult): void {
  ...
  if (result.reportPath) {
    console.log(pc.dim(`  Report: ${result.reportPath}`))
  }
  console.log('')
}
...
export function reportServerReused(url: string): void {
  console.log(pc.green(`Reusing existing server at ${url}`))
}
...
export function reportVerboseLog(line: StagehandLogLine): void {
  const msg = line.message
  if (!msg) return
  if (SUPPRESSED_LOG_PATTERNS.some(p => p.test(msg))) return

  const category = typeof line.data?.category === 'string' ? line.data.category : 'stagehand'
  reportVerbose(`[${category}] ${msg}`)
}
...
export interface ReporterContext {
  ...
  verboseLog(line: StagehandLogLine): void
  flush(): void
}
...
export function createBufferedReporter(): ReporterContext {
  ...
  return {
    ...
    verboseLog(line) {
      ...
      if (SUPPRESSED_LOG_PATTERNS.some(p => p.test(msg))) return
      const category = typeof line.data?.category === 'string' ? line.data.category : 'stagehand'
      lines.push(pc.dim(`      [${category}] ${msg}`))
    },
    flush() {
