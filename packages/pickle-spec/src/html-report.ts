...
function getScenarioDebugAssets(scenario: ScenarioResult): { screenshots: number; traceFrames: number } {
  let screenshots = 0
  let traceFrames = 0

  for (const step of scenario.steps) {
    if (step.screenshotPath) screenshots++
    if (step.traceFramePaths) traceFrames += step.traceFramePaths.length
  }

  return { screenshots, traceFrames }
}

function buildFailureSummary(result: RunResult): string {
  const failures: Array<{
    featureName: string
    scenarioName: string
    stepText: string
    error: string
    assets: { screenshots: number; traceFrames: number }
  }> = []
  ...
  let html = '<section class="failure-summary">'
  html += `<h2>Failure Summary <span class="failure-count">${failures.length}</span></h2>`
  html += '<div class="failure-list">'

  for (const failure of failures) {
    const assetLabels: string[] = []
    if (failure.assets.screenshots > 0) {
      assetLabels.push(`${failure.assets.screenshots} screenshot${failure.assets.screenshots === 1 ? '' : 's'}`)
    }
    if (failure.assets.traceFrames > 0) {
      assetLabels.push(`${failure.assets.traceFrames} trace frame${failure.assets.traceFrames === 1 ? '' : 's'}`)
    }

    html += '<article class="failure-item">'
    html += `<p class="failure-heading">${escapeHtml(failure.featureName)} / ${escapeHtml(failure.scenarioName)}</p>`
    html += `<p class="failure-step">Failed step: ${escapeHtml(failure.stepText)}</p>`
    html += `<pre class="failure-error">${escapeHtml(failure.error)}</pre>`
    if (assetLabels.length > 0) {
      html += `<p class="failure-assets">Debug assets: ${escapeHtml(assetLabels.join(', '))}</p>`
    }
    html += '</article>'
  }
...
function buildScenarioHtml(
  ...
  const assets = getScenarioDebugAssets(scenario)
  const assetBadges: string[] = []

  if (assets.screenshots > 0) {
    assetBadges.push(`<span class="artifact-badge">Screenshots: ${assets.screenshots}</span>`)
  }
  if (assets.traceFrames > 0) {
    assetBadges.push(`<span class="artifact-badge">Trace frames: ${assets.traceFrames}</span>`)
  }

  let html = `<details class="scenario ${status}"${isOpen}>`
  html += `<summary>${statusIcon(status)} <span class="scenario-name">${escapeHtml(scenario.pickle.name)}</span> ${assetBadges.join('')} <span class="duration">${formatDuration(scenario.durationMs)}</span></summary>`
...
function buildHtml(result: RunResult, imageCache: Map<string, string>): string {
  ...
  const failureSummaryHtml = buildFailureSummary(result)
