...
Feature: Search
  ...
  Scenario: Query the catalog
    ...
    try {
      ...
      await expect
        .poll(() => trigger.evaluate((element) => element.matches(':focus')))
        .toBe(true)
...
Feature: Search
  ...
  Scenario: Query the catalog
    ...
    try {
      ...
      await expect.poll(() => history.getByRole('row').count()).toBe(2)
...
`,
  ...
  test('Studio creates named test suites and execution target profiles', async () => {
    ...
    try {
      ...
      await page.getByRole('button', { name: 'smoke', exact: true }).waitFor()
